...
const customerTypeValues: [CustomerType, ...CustomerType[]] = [
  '成人', '儿童', '家庭', '孕产需求',
];

const regionValues: [MedicalRegion, ...MedicalRegion[]] = [
  '中国大陆', '港澳台', '全球除美加', '全球',
];

const hospitalValues: [HospitalType, ...HospitalType[]] = [
  '公立普通部', '特需部', '国际部', 'VIP部', '私立医院', '昂贵医院',
];

const outpatientValues: [OutpatientNeed, ...OutpatientNeed[]] = [
  '不需要', '需要普通门诊', '需要门诊直付',
];

const preExistingValues: [PreExistingCondition, ...PreExistingCondition[]] = [
  '无', '轻微异常', '明确既往症', '既往理赔记录',
];

const yesNoValues: [YesNo, ...YesNo[]] = ['是', '否'];
